...
# Note:
theNote=Label(frame2,text='Please manually check:',font=('Arial',10)).grid(sticky=W,row=2,column=0)
theNote_details1=Label(frame2,text='1.Pay attention to "empty resource rows" or "empty replicas" as it might be incorrect. \n',font=('Arial',10)).grid(sticky=W,row=3,column=0)

theNote_details2=Label(frame2,text='2.Check the entire empty services (both resources and replicas):                                                                                \n'
                                   '-eric-data-search-engine                                                                                                                           \n'
                                   ,font=('Arial',10)).grid(sticky=W,row=4,column=0)
...
theNote_details3=Label(frame2,text='4.Check the empty replicas:                                                                                                                                   \n'
                                   '-eric-sec-key-management, two groups of repica info, use PCX replicas=2?                                                                               \n'
                                   '-eric-log-transformer, two groups of replica info, confirm to use PCX replicas=2. Done!                                                                 \n'
                                   '-eric-odca-diagnostic-data-collector, Why ADP has two replicas (service.replicas:1, manualService.replicas:1)?                                          \n'
                                   '-eric-data-distributed-coordinator-ed, Why ADP has two replicas (Pods.dced.replicas:3, brAgent.replicas:1)?                                            \n'
                                   '-eric-lm-combined-server, Why ADP has two replicas (licenseConsumerHandler.replicaCount:2, licenseServerClient.replicaCount:1)?                         \n'
                                   ,font=('Arial',10)).grid(sticky=W,row=6,column=0)

# theNote_details3=Label(frame2,text='3.P:',font=('Arial',10)).grid(sticky=W,row=5,column=0)

theButton1 = Button(frame2, text='RUN', command=run_all_together,justify=RIGHT,default='active',font=('Arial', '10','bold'),anchor=W).grid(sticky=W, row=10,column=50)
