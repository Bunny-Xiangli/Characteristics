    ...
    def process(self,subfolder,data): #从yaml file里取出resource和replica的值
        ...
        if "resources" in data: #写这段儿是让resources不要返回None?
            ...
            for resource in resources:
                # global data_list_ADP
                self.data_list_ADP = []
                self.data_list_ADP.append(subfolder) # 第一列是subfolder，就是subfolders列表里的每一个值，也就是pod name，比如eric-log-transformer
                self.data_list_ADP.append(resource) # 第二列是resource，就是resources里的每个resource，比如logtransformer
                self.data_list_ADP.append(self.return_value(resources[resource],["requests","cpu"])) #调用return_value函数，输入为一个字典和不定的众多levels
                self.data_list_ADP.append(self.return_value(resources[resource], ["requests", "memory"]))
                self.data_list_ADP.append(self.return_value(resources[resource], ["requests", "ephemeral-storage"]))
                self.data_list_ADP.append(self.return_value(resources[resource], ["limits", "cpu"]))
                self.data_list_ADP.append(self.return_value(resources[resource], ["limits", "memory"]))
                self.data_list_ADP.append(self.return_value(resources[resource], ["limits", "ephemeral-storage"]))
                ...
                if replica is None:
                    self.data_list_ADP.append(replica_2nd)
                else:
                    self.data_list_ADP.append(replica)
                print(self.data_list_ADP)
                self.worksheet.append(self.data_list_ADP)
        else:
