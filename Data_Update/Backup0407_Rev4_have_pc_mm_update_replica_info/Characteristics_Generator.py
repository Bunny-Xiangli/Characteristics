...
#设置输入路径的默认选项,By Defaul
# helm_charts_path.set(r'C:\Users\lxi\OneDrive - Sigma Technology\Desktop\latest_eric-pc-gateway-1.47.0-1889\eric-pc-gateway-1.47.0-1889')
...
def run_all_together():
    ...
    if 'eric-pc-gateway' in os.listdir(Get_from_Entry_path):
        ...
        ADP_Run = ADP_Extracted.Extract_Data(ADP_PATH, subfolders, filename, Output_path,product)  # def里不能跑class么？
        ADP_Run.action()
...
theNote_details1=Label(frame2,text='1. The "MISMTACH" Excel sheet.\n',font=('Arial',10),justify=LEFT).grid(sticky=W,row=3,column=0)

theNote_details3=Label(frame2,text='2.For PCG, the following services containing multiple replicas in the ADP YAML file:\n'
...
theNote_details3 = Label(frame2,text='3.For PCG, though the following services have two groups of replica in "FINAL" sheet, but should be OK: \n'
...
theButton1 = Button(frame2, text='RUN', command=run_all_together,justify=RIGHT,default='active',font=('Arial', '10','bold'),anchor=E).grid(sticky=W, row=10,columnspan=10, column=50)
