    ...
    def loopfile(self): # 遍历subfolders，得到每个subfolder的绝对路径，并调用extract函数
        for subfolder in self.subfolders:
            if subfolder == 'eric-pc-sm':
                ...
                subfolder_charts = os.listdir(self.path + "/" + 'eric-pc-mm/charts')
                # print(subfolder_charts)
                for subsubfolder in subfolder_charts:
                    absolutepath = self.path + "/" + subfolder + "/" + 'charts/' + subsubfolder + '/' + self.filename
                    self.extract(subsubfolder, absolutepath)
                    # print (subsubfolder)
            else: #当不在pc-sm和pc-mm里时
        ...
        if isinstance(data, dict): #data是一个subfolder，即一个pod的字典数据
            ...
            for i in range(len(data)):
                ...
                if len(key_list_replica_value) == 0 and len(key_list_replicaCount_value) == 0:
                    ...
                    # Final_value.append('No replica info')
                    ...
                    Final_value = []
                    temp_list = ['Check two replica info', 'replicas', str(key_list_replica_value[0]), 'replicaCount', str(key_list_replicaCount_value[0])]
                    temp_str = '_'.join(temp_list)
                    Final_value.append(temp_str)

                elif len(key_list_replica_value) == 2:
                    # Final_value = key_list_replica_value[0] +key_list_replica_value[1]
                    Final_value = []
                    temp_list = ['Check two replicas',str(key_list_replica_value[0]), str(key_list_replica_value[1])]
                    temp_str = '_'.join(temp_list)
                    Final_value.append(temp_str)
                    ...
                    Final_value = []
                    temp_list = ['Check two replicaCounts',str(key_list_replicaCount_value[0]), str(key_list_replicaCount_value[1])]
                    temp_str = '_'.join(temp_list)
                    Final_value.append(temp_str)
