...
class Extract_Data_3rd_USER():
    ...
    def action(self,excel_path):
        ...
        self.worksheet = self.workbook.active
        self.worksheet = self.workbook.create_sheet('USER')
        # self.worksheet = self.workbook.create_sheet(title='USER', index=2,formatting_info=True)
        ...
        self.getfile()
        print("Done for getting USER data.")
    ...
    def process(self, podnamelist_PCG, data):  # 从yaml file里取出resource和replica的值
        for everypodname in podnamelist_PCG:
            if everypodname not in "eric-data-search-engine": #serch_engine的replica有两层级
    ...
    def get_detailed_resources(self, replica_1, replica_2,resources_level1, everypodname, container=None):  # 万一没有传参数，container=None, 默认的container值是None
        ...
        self.worksheet.append(data_resource_list) #每次跑一次这个def，就会贴一行到Excel里
