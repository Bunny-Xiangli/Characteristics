...
theLabel_Charts_example = Entry(frame1,textvariable=example1,width=150, bd=0,state="readonly").grid(row = 1,column = 0,sticky = W)
...
def run_all_together():
    ...
    if 'eric-pc-gateway' in os.listdir(Get_from_Entry_path):
        ...
        # print(ADP_Excel_path)
        # print(PCG_Excel_path)
        # print(USER_Excel_path)
...
# Note:
theNote=Label(frame2,text='Note: Please manually check:',font=('Arial',10,'bold')).grid(sticky=W,row=2,column=0)
theNote_details1=Label(frame2,text='1.The "empty resource rows" or "empty replicas". It may be incorrect because of different level in YAML file.',font=('Arial',10)).grid(sticky=W,row=3,column=0)
theNote_details2=Label(frame2,text='2.For PCG, the following services:                        \n'
                                   '-Search Engine (excluded as different format)\n'
                                   'Data'
...
theButton1 = Button(frame2, text='RUN', command=run_all_together,justify=RIGHT,default='active',font=('Arial', '10','bold')).grid(sticky=W, row=20,column=30)
