...
class Test ():
    ...
    def process(self,podnamelist_PCG,data): #从yaml file里取出resource和replica的值
        for everypodname in podnamelist_PCG:
            ...
            print (type(everypodname_2ndhalf))
            # replicaCount: 2
            # resources:
            # logtransformer:
            # limits:
            ...
            if ("requests" or "limits") in resources_details:
                self.get_detailed_resources(resources_details,everypodname) # 传的是requests/limits这一层
            else:
                for container in resources_details:
                    # resources_details[container] # key对应value
                    self.get_detailed_resources(resources_details[container],everypodname,container) #传的是requests/limits这一层

    def get_detailed_resources(self, resources_level1, everypodname, container=None): # 万一没有传参数，container=None, 默认的container值是None
        data_list = []
...
if __name__ == '__main__':
    extracted = Extracted.Extract_Data(settings.PATH,settings.subfolders,settings.filename,settings.excel_path)
    extracted.action()
